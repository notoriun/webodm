...
def getShortLinks(username):
    return ds.get_json(
        username + "_shortlinks",
        {"t": {}, "i": {}},  # task --> short id  # short id --> task
    )
    ...
    def post(self, request, pk=None):
        ...
        if not re.match(r"^[A-Za-z0-9_-]+$", short_id):
            return Response(
                {
                    "error": _(
                        "Short URLs can only include letters, numbers, underscore and dash characters (A-Z, 0-9, _, -)."
                    )
                }
            )
        ...
        return Response(
            {"username": username, "shortId": short_id}, status=status.HTTP_200_OK
        )
...
class GetShortLink(TaskView):
    ...
    def post(self, request, pk=None):
        ...
        if task_id in shortlinks["t"]:
            ...
            return Response(
                {"username": username, "shortId": shortlinks["t"][task_id]},
