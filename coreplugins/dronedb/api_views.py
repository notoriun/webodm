...
def is_valid(file):
    ...
    return (
        file_extension.lower() in VALID_IMAGE_EXTENSIONS
        or file == "gcp_list.txt"
        or file == "geo.txt"
    )
...
def to_web_protocol(registry_url):
    return (
        registry_url.replace("ddb+unsafe://", "http://")
        .replace("ddb://", "https://")
        .rstrip("/")
    )
...
class CheckCredentialsTaskView(TaskView):
    ...
    def post(self, request):
        ...
        if hub_url == None or username == None or password == None:
            return Response(
                {"error": "All fields must be set."}, status=status.HTTP_400_BAD_REQUEST
            )
...
class FoldersTaskView(TaskView):
    ...
    def get(self, request, org=None, ds=None):
        ...
        if org == None or ds == None:
            return Response(
                {"error": "Organization and dataset must be set."},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class VerifyUrlTaskView(TaskView):
    ...
    def post(self, request):
        ...
        if url == None:
            return Response(
                {"error": "Url must be set."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            ...
            result, org, ds, folder, count, size = verify_url(
...
class InfoTaskView(TaskView):
    ...
    def get(self, request):
        ...
        return Response(
            {"hubUrl": registry_url, "username": username}, status=status.HTTP_200_OK
        )
    ...
    def post(self, request, project_pk=None, pk=None):
        ...
        datastore.set_json(
            combined_id,
            {
                ...
                "token": ddb.token,
                "ddbWebUrl": "{}/r/{}/{}/{}".format(
                    to_web_protocol(registry_url), orgSlug, dsSlug, folder.rstrip("/")
                ),
            },
        )
...
def import_files(task_id, carrier):
    ...
    def download_file(task, file):
        ...
        download_stream = requests.get(
            file["url"], stream=True, timeout=60, headers=headers
        )
...
class CheckUrlTaskView(TaskView):
    ...
    def get(self, request, project_pk=None, pk=None):
        ...
        data = (
            ...
            .get_json(combined_id, default=None)
        )
...
class ShareTaskView(TaskView):
    ...
    def post(self, request, pk):
        ...
        available_assets_names = task.list_available_assets_names()

        available_assets = [
            task.get_asset_file_or_stream(f)
            for f in list(set(available_assets_names) & set(DRONEDB_ASSETS))
        ]

        if "textured_model.zip" in available_assets_names:
            texture_files = [
        ...
        files = [
            {"path": f, "name": f[len(assets_path) + 1 :], "size": os.path.getsize(f)}
    ...
    for file in files:
        ...
        while attempt < 3:
            try:
                ...
                if attempt == 3:
                    logger.error(
                        "Error uploading file {}: {}".format(file["name"], str(e))
                    )
