...
def calc_contours(task_id, dem, epsg, interval, output_format, simplify, zfactor=1):
    ...
    tmpdir = os.path.join(
        settings.MEDIA_TMP,
        os.path.basename(tempfile.mkdtemp("_contours", dir=settings.MEDIA_TMP)),
    )
    ...
    p = subprocess.Popen(
        [
            ...
            "sqlite",
            "-sql",
            f"SELECT ID, ROUND(level * {zfactor}, 5) AS level, GeomFromGML(AsGML(ATM_Transform(GEOM, ATM_Scale(ATM_Create(), 1, 1, {zfactor})), 10)) as GEOM FROM contour WHERE ST_Length(GEOM) >= {MIN_CONTOUR_LENGTH}",
        ],
        cwd=tmpdir,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
    ...
    def post(self, request, pk=None):
        ...
        try:
            ...
            celery_task_id = run_function_async(
                calc_contours, pk, dem, epsg, interval, format, simplify, zfactor
            ).task_id
            return Response(
                {"celery_task_id": celery_task_id}, status=status.HTTP_200_OK
            )
