...
import ImportFromS3TaskPanel from "./ImportFromS3TaskPanel";
...
class ProjectListItem extends React.Component {
  ...
  constructor(props) {
    ...
    this.handleTaskS3Saved = this.handleTaskS3Saved.bind(this);
  }
        ...
        .on("uploadprogress", (file, progress, bytesSent) => {
          ...
          if (progress === 100 || now - this.state.upload.lastUpdated > 500) {
            ...
            const totalBytesSent =
              this.state.upload.totalBytesSent + deltaBytesSent;
            const progress =
          ...
          const retry = () => {
            ...
            if (file.retries < MAX_RETRIES) {
              ...
              throw new Error(
                interpolate(
                  _(
                    "Cannot upload %(filename)s, exceeded max retries (%(max_retries)s)"
                  ),
                  { filename: file.name, max_retries: MAX_RETRIES }
                )
              );
            }
          };
            ...
            if (file.status === "error") {
              if (file.size / 1024 / 1024 > this.dz.options.maxFilesize) {
                ...
                throw new Error(
                  interpolate(
                    _(
                      "Cannot upload %(filename)s, file is too large! Default MaxFileSize is %(maxFileSize)s MB!"
                    ),
                    {
                      filename: file.name,
                      maxFileSize: this.dz.options.maxFilesize,
                    }
                  )
                );
              }
        ...
        .on("queuecomplete", () => {
          ...
          if (
            ...
            $.ajax({
              ...
              .done((task) => {
                if (task && task.id) {
                  ...
                  this.setUploadState({
                    error: interpolate(
                      _(
                        "Cannot create new task. Invalid response from server: %(error)s"
                      ),
                      { error: JSON.stringify(task) }
                    ),
                  });
                }
              })
              .fail(() => {
                this.setUploadState({
                  error: _("Cannot create new task. Please try again later."),
                });
              });
            ...
            this.setUploadState({
              ...
              error: interpolate(
                _(
                  "%(count)s files cannot be uploaded. As a reminder, only images (.jpg, .tif, .png) and GCP files (.txt) can be uploaded. Try again."
                ),
  ...
  newTaskAdded = () => {
    this.setState({ importing: false, importingFromS3: false });
  ...
  toggleTaskList() {
    ...
    this.historyNav.toggleQSListItem(
      "project_task_open",
      this.state.data.id,
      showTaskList
    );
  ...
  handleTaskS3Saved(saveData) {
    const { taskInfo, taskEvents } = saveData;
    ...
    this.createNewTask(formData, taskEvents);
  }
  ...
  handleImportFromS3Task = () => {
    this.setState({ importing: false, importingFromS3: true });
  };
  ...
  handleCancelImportFromS3Task = () => {
    this.setState({ importingFromS3: false });
  };
          ...
          .then((gps) => {
            ...
            $.ajax({
              ...
              .done((json) => {
                ...
                else if (json.address && json.address.road)
                  resolve(`${json.address.road} - ${dateTime}`);
  ...
  toggleTag = (t) => {
    return () => {
      if (this.state.selectedTags.indexOf(t) === -1) {
        ...
        this.setState({
          selectedTags: this.state.selectedTags.filter((tag) => tag !== t),
        });
      }
    };
  };
  ...
  createNewTask = (formData, taskEvents) => {
    $.ajax({
    ...
    return (
      ...
      >
        ...
        <div className="row no-margin">
          ...
          <div className="btn-group project-buttons">
            {this.hasPermission("add") ? (
              <div
                className={
                  "asset-download-buttons btn-group " +
                  (this.state.upload.uploading ? "hide" : "")
                }
              >
                <button
                  type="button"
                ...
                <button
                  type="button"
                  className="btn btn-default btn-sm"
                  onClick={this.handleImportFromS3Task}
                >
                  <i className="glyphicon glyphicon-cloud-download"></i>{" "}
                  <span className="hidden-xs">{_("Import From S3")}</span>
                </button>
              ...
              <span>
                ...
                <a href="#" onClick={this.toggleTaskList}>
                  ...
                  <i
                    className={
                      "fa fa-caret-" +
                      (this.state.showTaskList ? "down" : "right")
                    }
                  ></i>
                </a>
              </span>
                ...
                <div className="btn-group">
                  ...
                  <ul className="dropdown-menu dropdown-menu-right filter-dropdown">
                    <li className="filter-text-container">
                      <input
                        type="text"
                        className="form-control filter-text theme-border-secondary-07"
                        value={this.state.filterText}
                        ref={(domNode) => {
                          this.filterTextInput = domNode;
                        }}
                        placeholder=""
                        spellCheck="false"
                        autoComplete="false"
                    ...
                    {filterTags.map((t) => (
                      <li key={t} className="tag-selection">
                        <input
                          type="checkbox"
                          className="filter-checkbox"
                          id={"filter-tag-" + data.id + "-" + t}
                          checked={this.state.selectedTags.indexOf(t) !== -1}
                          onChange={this.toggleTag(t)}
                        />{" "}
                        <label
                          className="filter-checkbox-label"
                          htmlFor={"filter-tag-" + data.id + "-" + t}
                        >
                          {t}
                        </label>
                      </li>
        ...
        <div className="row">
          ...
          {this.state.importingFromS3 ? (
            <ImportFromS3TaskPanel
              onImported={this.newTaskAdded}
              projectId={this.state.data.id}
              onSave={this.handleTaskS3Saved}
              onCancel={this.handleCancelImportFromS3Task}
              suggestedTaskName={this.handleTaskTitleHint}
              filesCount={this.state.upload.totalCount}
              showResize={true}
              getFiles={() => this.state.upload.files}
