    ...
    if project_pk is not None:
        ...
        if task_pk is not None:
            task = get_object_or_404(
                Task.objects.defer("orthophoto_extent", "dsm_extent", "dtm_extent"),
                pk=task_pk,
                project=project,
            )
...
def about(request):
    return render(
        request, "app/about.html", {"title": _("About"), "version": settings.VERSION}
    )
...
def processing_node(request, processing_node_id):
    ...
    if not pn.update_node_info():
        messages.add_message(
            ...
            _("%(node)s seems to be offline.") % {"node": pn},
        )

    return render(
        request,
        "app/processing_node.html",
        {
...
def welcome(request):
    ...
    return render(
        request, "app/welcome.html", {"title": _("Welcome"), "firstuserform": fuf}
    )
