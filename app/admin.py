...
class TaskAssetAdmin(admin.ModelAdmin):
    ...
    list_filter = ("status", "task", "status")
    search_fields = ("id", "name", "task__name")


admin.site.register(TaskAsset, TaskAssetAdmin)
...
class PluginAdmin(admin.ModelAdmin):
    ...
    def description(self, obj):
        manifest = get_plugin_by_name(
            obj.name, only_active=False, refresh_cache_if_none=True
        ).get_manifest()
    ...
    def plugin_enable(self, request, plugin_name, *args, **kwargs):
        try:
            ...
            if p.requires_restart():
                messages.warning(
                    request,
                    _("Restart required. Please restart WebODM to enable %(plugin)s")
                    % {"plugin": plugin_name},
                )
        except Exception as e:
            messages.warning(
                request,
                _("Cannot enable plugin %(plugin)s: %(message)s")
                % {"plugin": plugin_name, "message": str(e)},
            )
        ...
        try:
            ...
            if p.requires_restart():
                messages.warning(
                    request,
                    _(
                        "Restart required. Please restart WebODM to fully disable %(plugin)s"
                    )
                    % {"plugin": plugin_name},
                )
        except Exception as e:
            messages.warning(
                request,
                _("Cannot disable plugin %(plugin)s: %(message)s")
                % {"plugin": plugin_name, "message": str(e)},
            )
    ...
    def plugin_delete(self, request, plugin_name, *args, **kwargs):
        try:
            ...
            messages.warning(
                request,
                _("Cannot delete plugin %(plugin)s: %(message)s")
                % {"plugin": plugin_name, "message": str(e)},
            )
    ...
    def plugin_upload(self, request, *args, **kwargs):
        ...
        if file is not None:
            ...
            try:
                ...
                messages.warning(
                    request, _("Cannot load plugin: %(message)s") % {"message": str(e)}
                )
    ...
    def plugin_actions(self, obj):
        ...
        return format_html(
            ...
            '<a class="button" href="{}" {}>{}</a>'
            + (
                '&nbsp;<a class="button" href="{}" onclick="return confirm(\'Are you sure you want to delete {}?\')"><i class="fa fa-trash"></i></a>'
