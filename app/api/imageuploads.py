                ...
                points.append(
                    {
                        ...
                        "y": coords[1],
                        "color": (
                            hex2rgb(point_colors[i])
                            if i < len(point_colors)
                            else (255, 255, 255)
                        ),
                        "radius": (
                            float(point_radiuses[i]) if i < len(point_radiuses) else 1.0
                        ),
                    }
                )
        ...
        with Image.open(image_data) as img:
            ...
            for p in points:
                ...
                d.ellipse(
                    [(x - r, y - r), (x + r, y + r)],
                    outline=p["color"],
                    width=int(max(1.0, math.floor(r / 3.0))),
                )
...
class ImageDownload(TaskNestedView):
    ...
    def get(self, request, pk=None, project_pk=None, image_filename=""):
        ...
        return download_file_stream(
            request, image_stream, "attachment", os.path.basename(image_filename)
        )
